...
import sing.camera.library.listener.CaptureListener;
import sing.camera.library.listener.ClickListener;
import sing.camera.library.listener.ErrorListener;
import sing.camera.library.listener.SCameraListener;
import sing.camera.library.listener.TypeListener;
import sing.camera.library.state.CameraMachine;
import sing.camera.library.util.FileUtil;
import sing.camera.library.util.ScreenUtils;
import sing.camera.library.view.CameraView;

public class SCameraView extends FrameLayout implements CameraInterface.CameraOpenOverCallback, SurfaceHolder.Callback, CameraView {
    ...
    //回调监听
    private SCameraListener sCameraListener;
    ...
    private int cameraMarginLeft = 0;     // 切换按钮左边距
    private int cameraMarginTop = 0;     //切换按钮上边距
    private int cameraMarginRight = 0;     //切换按钮右边距
    private int cameraMarginBottom = 0;     //切换按钮下边距
    private int iconSrc = 0;        //图标资源
    private int iconLeft = 0;       //左图标
    private int iconRight = 0;      //右图标
    private int duration = 0;       //录制时间
    private Boolean hintFlashLamp = false;       // 隐藏闪光灯 默认不隐藏
    ...
    public SCameraView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SCameraView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        mContext = context;
        //get AttributeSet
        TypedArray a = context.getTheme().obtainStyledAttributes(attrs, R.styleable.SCameraView, defStyleAttr, 0);
        iconSize = a.getDimensionPixelSize(R.styleable.SCameraView_iconSize, (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, 35, getResources().getDisplayMetrics()));
        cameraMarginLeft = a.getDimensionPixelSize(R.styleable.SCameraView_cameraMarginLeft, (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, 15, getResources().getDisplayMetrics()));
        cameraMarginTop = a.getDimensionPixelSize(R.styleable.SCameraView_cameraMarginTop, (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, 15, getResources().getDisplayMetrics()));
        cameraMarginRight = a.getDimensionPixelSize(R.styleable.SCameraView_cameraMarginRight, (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, 15, getResources().getDisplayMetrics()));
        cameraMarginBottom = a.getDimensionPixelSize(R.styleable.SCameraView_cameraMarginBottom, (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, 15, getResources().getDisplayMetrics()));
        iconSrc = a.getResourceId(R.styleable.SCameraView_iconSrc, R.drawable.ic_camera);
        iconLeft = a.getResourceId(R.styleable.SCameraView_iconLeft, 0);
        iconRight = a.getResourceId(R.styleable.SCameraView_iconRight, 0);
        duration = a.getInteger(R.styleable.SCameraView_duration_max, 10 * 1000);       //没设置默认为10s
        hintFlashLamp = a.getBoolean(R.styleable.SCameraView_hintFlashLamp, false);
        a.recycle();
        initData();
        initView();
    }
    ...
    private void initView() {
        ...
        LinearLayout.LayoutParams params1 = (LinearLayout.LayoutParams) mSwitchCamera.getLayoutParams();
        params1.width = iconSize;
        params1.height = iconSize;
        params1.setMargins(cameraMarginLeft,cameraMarginTop,cameraMarginRight,cameraMarginBottom);
        mSwitchCamera.setLayoutParams(params1);

        mFlashLamp = view.findViewById(R.id.image_flash);
        LinearLayout.LayoutParams params2 = (LinearLayout.LayoutParams) mFlashLamp.getLayoutParams();
        params2.width = iconSize;
        params2.height = iconSize;
        mFlashLamp.setLayoutParams(params2);

        mFlashLamp.setVisibility(hintFlashLamp ? INVISIBLE : VISIBLE);
        setFlashRes();
    ...
    public void surfaceCreated(SurfaceHolder holder) {
        new Thread() {
            @Override
            public void run() {
                CameraInterface.getInstance().doOpenCamera(SCameraView.this);
            }
        }.start();
    }
    ...
    public void setSCameraListener(SCameraListener sCameraListener) {
        this.sCameraListener = sCameraListener;
    }
    ...
    public void confirmState(int type) {
        switch (type) {
            case TYPE_VIDEO:
                ...
                mVideoView.setLayoutParams(new LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT));
                machine.start(mVideoView.getHolder(), screenProp);
                if (sCameraListener != null) {
                    sCameraListener.recordSuccess(videoUrl, firstFrame);
                }
                break;
            case TYPE_PICTURE:
                mPhoto.setVisibility(INVISIBLE);
                if (sCameraListener != null) {
                    sCameraListener.captureSuccess(captureBitmap);
                }
                break;
            case TYPE_SHORT:
                break;
...
//            mPhoto.setScaleType(ImageView.ScaleType.FIT_XY);
    ...
    public void playVideo(Bitmap firstFrame, final String url) {
        videoUrl = url;
        SCameraView.this.firstFrame = firstFrame;
